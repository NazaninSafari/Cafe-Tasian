...
$res = mysqli_query($link, "SELECT * FROM product WHERE pro_code=$id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $img   = $product['pro_image']; // حفظ تصویر فعلی
...
<title>ویرایش محصول – ادمین</title>
...
.field input{width:100%;background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:11px 14px;color:var(--text);font-family:inherit;font-size:.9rem;outline:none;transition:border-color .2s;}
.field input:focus{border-color:rgba(200,169,110,.5);}
...
.current-img{display:flex;align-items:center;gap:12px;background:var(--surface2);padding:12px;border-radius:10px;border:1px solid var(--border);margin-bottom:10px;}
.current-img img{width:60px;height:60px;object-fit:cover;border-radius:8px;}
.current-img span{font-size:.82rem;color:var(--muted);}
...
<main class="main">
  <div class="topbar">
    ...
    <div class="page-title">ویرایش: <?= htmlspecialchars($product['pro_name']) ?></div>
  </div>
  ...
  <div class="form-card">
    <form method="POST" enctype="multipart/form-data">
      <div class="field">
        ...
        <input type="text" name="pro_name" value="<?= htmlspecialchars($product['pro_name']) ?>">
      </div>
      <div class="row2">
        <div class="field">
          <label>قیمت (تومان) *</label>
          <input type="number" name="pro_price" min="0" value="<?= $product['pro_price'] ?>">
        </div>
        <div class="field">
          <label>موجودی انبار *</label>
          <input type="number" name="pro_qty" min="0" value="<?= $product['pro_qty'] ?>">
        </div>
      </div>
      <div class="field">
        <label>تصویر محصول (اختیاری — برای تغییر تصویر فعلی)</label>
        <?php if (!empty($product['pro_image'])): ?>
        <div class="current-img">
          <img src="../images/products/<?= htmlspecialchars($product['pro_image']) ?>" alt="">
          <span>تصویر فعلی: <?= htmlspecialchars($product['pro_image']) ?></span>
        </div>
        <?php endif; ?>
